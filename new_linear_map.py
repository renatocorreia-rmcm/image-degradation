...
def rotate(img: np.ndarray, angle: float = (2 * np.pi) / 360, vertices=None):
    return linear_map(
        matrix=np.array([
...
def linear_map(matrix: np.ndarray, img: np.ndarray, vertices_pixels: np.ndarray = None):
    """
    ...
    def get_p(v):
        ...
        map v to p
        """
        (a, b) = vertices_pixels[0]  # (0, 0) pra imagem enquadrada
        # p = v + (a, b)
    ...
    for i_ in range(h_):
        for j_ in range(w_):
            ...
            if (
                ...
                # color = interpolation(img, i, j, h, w, fl)
                color = img[int(p[0]), int(p[1])]  # knn interp for debug
...
if __name__ == "__main__":
    ...
    new_img, vertices_ = linear_map(A, v)
    cv2.imwrite('new_img.png', new_img)
    B =np.linalg.inv(A)
    for i in range(50):
        print(i)
        new_img, vertices_ = linear_map(A, new_img, vertices_pixels=vertices_)
        new_img, vertices_ = linear_map(B, new_img, vertices_pixels=vertices_)
        cv2.imwrite(f'new_img{i}.png', new_img)
